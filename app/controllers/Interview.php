...
class Interview extends CI_Controller {
	...
	public function __construct() {
        ...
        $this->load->model('Interview_model','model');
    }
    function index()
	{
		$jquery = "$(function () {
    ...
    function update(){
		if($this->uri->segment(3) == "status"){
			...
			if($this->input->post("penilaian") == 1){
				//generate cert pdf
				$path = $this->generatecert($this->input->post('userid'),$datecomplete);
			}
			/* echo $path;
		exit; */	
			$this->model->updatestudentstatus($path,$datecomplete);
	...
	public function generatecert($u_id,$datecomplete){
		...
		$insid = $this->blapps_lib->getData("ins_id","tb_users","u_id",base64_decode($u_id));
		...
		$data['tarikhtamat'] = $datecomplete;
		$this->load->view("interview/certpdf",$data);
